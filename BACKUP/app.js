...
function initApp() {
    // 1. Cargar Usuarios
    const storedUsers = localStorage.getItem("tm_caja_usuarios");
    if (!storedUsers) {
        state.usuarios = [...SEED_USERS];
        saveUsuariosToLocalStorage();
    } else {
        state.usuarios = JSON.parse(storedUsers);
        // Asegurarse de que el usuario operador semilla exista para facilitar la demo
        const hasOperator = state.usuarios.some(u => u.usuario === "operador");
        if (!hasOperator) {
            state.usuarios.push({ id: "u3", nombre: "Camilo (Operador Caja)", usuario: "operador", contrasena: "caja", rol: "Operador de Módulo", modulo: "modulo1" });
            saveUsuariosToLocalStorage();
        }
    }

    // 2. Cargar Datos Financieros
    const storedM0 = localStorage.getItem("tm_caja_modulo0");
    const storedM1 = localStorage.getItem("tm_caja_modulo1");
    const storedM2 = localStorage.getItem("tm_caja_modulo2");
    const storedM3 = localStorage.getItem("tm_caja_modulo3");

    if (!storedM0 || !storedM1 || !storedM2 || !storedM3) {
        state.modulo0 = [...SEED_DATA.modulo0];
        state.modulo1 = [...SEED_DATA.modulo1];
        state.modulo2 = [...SEED_DATA.modulo2];
        state.modulo3 = [...SEED_DATA.modulo3];
        saveStateToLocalStorage();
    } else {
        state.modulo0 = JSON.parse(storedM0);
        state.modulo1 = JSON.parse(storedM1);
        state.modulo2 = JSON.parse(storedM2);
        state.modulo3 = JSON.parse(storedM3);
    }

    // 3. Cargar Sesión de Usuario
    const storedSession = localStorage.getItem("tm_caja_currentUser");
    if (storedSession) {
        state.currentUser = JSON.parse(storedSession);
        // Validar si el usuario aún existe en la base de datos local
        const userExists = state.usuarios.find(u => u.id === state.currentUser.id);
        if (userExists) {
            state.currentUser = userExists; // sincronizar cambios
            showAppLayout();
        } else {
            logoutUser();
        }
...
function saveStateToLocalStorage() {
    try {
        localStorage.setItem("tm_caja_modulo0", JSON.stringify(state.modulo0));
        localStorage.setItem("tm_caja_modulo1", JSON.stringify(state.modulo1));
        localStorage.setItem("tm_caja_modulo2", JSON.stringify(state.modulo2));
        localStorage.setItem("tm_caja_modulo3", JSON.stringify(state.modulo3));
    } catch (error) {
        console.error("Error al guardar en localStorage:", error);
        showToast("Error: No hay suficiente espacio en el navegador para guardar los datos. Elimina imágenes de soporte grandes.", "danger");
    }
}

function saveUsuariosToLocalStorage() {
    try {
        localStorage.setItem("tm_caja_usuarios", JSON.stringify(state.usuarios));
    } catch (error) {
        console.error("Error al guardar usuarios en localStorage:", error);
        showToast("Error: No hay suficiente espacio en el navegador para guardar los datos de usuario.", "danger");
    }
}
...
function logoutUser() {
    ...
    localStorage.removeItem("tm_caja_currentUser");
    showLoginLayout();
...
function setupSessionHandlers() {
    ...
    document.getElementById("login-form").addEventListener("submit", (e) => {
        ...
        const foundUser = state.usuarios.find(u => u.usuario.toLowerCase() === usernameVal && u.contrasena === passwordVal);

        if (foundUser) {
            state.currentUser = foundUser;
            localStorage.setItem("tm_caja_currentUser", JSON.stringify(foundUser));
            showToast(`Bienvenido(a), ${foundUser.nombre}.`, "success");
            showAppLayout();
        } else {
...
function deleteM0(id) {
    const countM1 = state.modulo1.filter(item => item.entregado_a === id).length;
    const countM2 = state.modulo2.filter(item => item.se_debe_a === id).length;
    const countM3 = state.modulo3.filter(item => item.quien_entrega === id || item.entregado_a === id).length;
    const totalUses = countM1 + countM2 + countM3;

    let warningMsg = "¿Está seguro de que desea eliminar este colaborador?";
    if (totalUses > 0) {
        warningMsg = `ATENCIÓN: Este colaborador está asociado a ${totalUses} registros financieros. Si lo elimina, aparecerá como "(Colaborador No Registrado)" en el historial. ¿Desea continuar?`;
    }

    if (confirm(warningMsg)) {
        state.modulo0 = state.modulo0.filter(item => item.id !== id);
        saveStateToLocalStorage();
        populateTercerosSelects();
        renderAll();
...
function deleteM1(id) {
    if (confirm("¿Está seguro de que desea eliminar este registro de caja?")) {
        state.modulo1 = state.modulo1.filter(item => item.id !== id);
        saveStateToLocalStorage();
        renderAll();
...
function deleteM2(id) {
    if (confirm("¿Está seguro de que desea eliminar este pago pendiente?")) {
        state.modulo2 = state.modulo2.filter(item => item.id !== id);
        saveStateToLocalStorage();
        renderAll();
...
function toggleReportM2(id) {
    const record = state.modulo2.find(item => item.id === id);
    if (!record) return;
    record.reportado = !record.reportado;
    saveStateToLocalStorage();
    renderAll();
    const statusText = record.reportado ? "marcado como Reportado" : "marcado como Pendiente";
...
function deleteM3(id) {
    if (confirm("¿Está seguro de que desea eliminar este registro de ventas?")) {
        state.modulo3 = state.modulo3.filter(item => item.id !== id);
        saveStateToLocalStorage();
        renderAll();
...
function quickConfirmDeliveryM3(id) {
    const record = state.modulo3.find(item => item.id === id);
    if (!record) return;
    
    const tesoreria = state.modulo0.find(item => item.nombre.includes("Tesorería"));
    record.estado_entrega = true;
    record.entregado_a = tesoreria ? tesoreria.id : "t2";
    record.fecha_entrega = new Date().toISOString().split('T')[0];
...
function setupFormHandlers() {
    ...
    document.getElementById("user-form").addEventListener("submit", (e) => {
        ...
        // Validar nombre de usuario único al crear
        if (!id) {
            const userExists = state.usuarios.find(u => u.usuario.toLowerCase() === record.usuario);
            if (userExists) {
                showToast(`El nombre de usuario "${record.usuario}" ya está en uso.`, "warning");
                return;
            }
        }
        ...
        if (id) {
            ...
            if (index !== -1) {
                state.usuarios[index] = { ...state.usuarios[index], ...record };
                // Sincronizar usuario actual si editó su propia cuenta
                if (state.currentUser.id === id) {
                    state.currentUser = state.usuarios[index];
                    localStorage.setItem("tm_caja_currentUser", JSON.stringify(state.currentUser));
                    applyRolePermissions();
                }
                showToast("Usuario actualizado.", "success");
            }
        } else {
            record.id = "u_" + Date.now();
            state.usuarios.push(record);
    ...
    document.getElementById("m0-form").addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        if (id) {
            const index = state.modulo0.findIndex(item => item.id === id);
            if (index !== -1) {
                state.modulo0[index] = { ...state.modulo0[index], ...record };
                showToast("Colaborador actualizado.", "success");
            }
        } else {
            record.id = "t_" + Date.now();
            state.modulo0.push(record);
    ...
    document.getElementById("quick-tercero-form").addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        state.modulo0.push(newTercero);
        saveStateToLocalStorage();
        populateTercerosSelects();
        
        const selectEl = document.getElementById(targetSelectId);
        if (selectEl) {
            selectEl.value = newTercero.id;
        }
    ...
    document.getElementById("m1-form").addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        if (id) {
            const index = state.modulo1.findIndex(item => item.id === id);
            if (index !== -1) {
                state.modulo1[index] = { ...state.modulo1[index], ...record };
                showToast("Registro de caja actualizado.", "success");
            }
        } else {
            record.id = "m1_" + Date.now();
            state.modulo1.push(record);
            showToast("Registro de caja guardado.", "success");
        }
    ...
    document.getElementById("m2-form").addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        if (id) {
            const index = state.modulo2.findIndex(item => item.id === id);
            if (index !== -1) {
                state.modulo2[index] = { ...state.modulo2[index], ...record };
                showToast("Pago pendiente actualizado.", "success");
            }
        } else {
            record.id = "m2_" + Date.now();
            state.modulo2.push(record);
            showToast("Pago pendiente registrado.", "success");
        }
    ...
    document.getElementById("m3-form").addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        if (id) {
            const index = state.modulo3.findIndex(item => item.id === id);
            if (index !== -1) {
                state.modulo3[index] = { ...state.modulo3[index], ...record };
                showToast("Registro de ventas actualizado.", "success");
            }
        } else {
            record.id = "m3_" + Date.now();
            state.modulo3.push(record);
            showToast("Recaudo de ventas registrado.", "success");
        }
...
function setupBackupHandlers() {
    ...
    document.getElementById("db-btn-export").addEventListener("click", () => {
        ...
        const jsonString = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(fullDB, null, 2));
        const downloadAnchor = document.createElement('a');
        downloadAnchor.setAttribute("href", jsonString);
        downloadAnchor.setAttribute("download", `CajaMenor_V3_Backup_${new Date().toISOString().slice(0, 10)}.json`);
            ...
            try {
                ...
                if (parsed.base_de_datos && parsed.base_de_datos.modulo0 && parsed.base_de_datos.modulo1 && parsed.base_de_datos.modulo2 && parsed.base_de_datos.modulo3) {
                    if (confirm("¿Desea restaurar esta base de datos? Se sobrescribirá toda la información actual.")) {
                        // Restaurar también usuarios si están en la base de datos importada, sino conservar actuales
                        if (parsed.base_de_datos.usuarios) {
                            state.usuarios = parsed.base_de_datos.usuarios;
                            saveUsuariosToLocalStorage();
                        }
                        
                        state.modulo0 = parsed.base_de_datos.modulo0;
                        state.modulo1 = parsed.base_de_datos.modulo1;
                        state.modulo2 = parsed.base_de_datos.modulo2;
                        state.modulo3 = parsed.base_de_datos.modulo3;
                        
                        saveStateToLocalStorage();
                        populateTercerosSelects();
                        renderAll();
                        
                        // Sincronizar sesión si cambió
                        const userExists = state.usuarios.find(u => u.id === state.currentUser.id);
                        if (!userExists) {
                            logoutUser();
                        } else {
    ...
    // Reiniciar
    document.getElementById("db-btn-reset-demo").addEventListener("click", () => {
        if (state.currentUser && state.currentUser.rol === "Visualización") return;
    ...
    // Vaciar
    document.getElementById("db-btn-clear-all").addEventListener("click", () => {
        if (state.currentUser && state.currentUser.rol === "Visualización") return;
